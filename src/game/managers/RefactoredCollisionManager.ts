...
import { LaserCollisionAdapter, AsteroidCollisionAdapter } from '../collision/adapters/ProjectileCollisionAdapters';
...
export class RefactoredCollisionManager {
    ...
    private registerPlayerShips(): void {
        ...
        if (modularShip) {
            ...
            console.log(`📝 Registered player ship ${modularShip.id} as collision target`);
        }
    }
    ...
    private registerAIShips(): void {
        ...
        for (const aiShip of aiShips) {
            if (aiShip.ship && typeof aiShip.ship.takeDamageAtComponentId === 'function') {
                const adapter = new ModularShipCollisionAdapter(aiShip.ship);
                this.collisionSystem.registerTarget(adapter);
                this.entityAdapters.set(aiShip.ship.id, adapter);
                console.log(`📝 Registered AI ship ${aiShip.ship.id} as collision target`);
            }
        }
    }
    /**
    ...
    private registerAsteroids(): void {
        ...
        console.log(`📝 Registered ${asteroids.length} asteroids as collision sources`);
    }
    /**
