...
export class GenericCollisionSystem {
  ...
  handleCollision(event: CollisionEvent): void {
    ...
    if (!entityA || !entityB) {
      console.log(
        `❌ Could not find entities for collision bodies: ${bodyA.id}, ${bodyB.id}`
      );
      return;
    }
    ...
    if (!collision) {
      console.log(
        `ℹ️ No damage relationship between ${entityA.id} and ${entityB.id}`
      );
      return;
    }
    ...
    const collisionInfo = this.infoExtractor.extractCollisionInfo(
      event,
      {
        ...
        physicsBodyId: collision.source.physicsBodyId,
      },
      { id: collision.target.id, physicsBodyId: collision.target.physicsBodyId }
    );

    // Resolve collision
    const result = this.resolver.resolveCollision(
      collision.source,
      collision.target,
      collisionInfo
    );
