...
export class CollisionResolver {
    ...
    resolveCollision(
        ...
        console.log(`🎯 Resolving collision: ${source.sourceType} ${source.id} -> ${target.id}`);

        // Check for friendly fire
        if (!target.shouldTakeDamageFrom(source.sourceId, source.sourceType)) {
            console.log(`🚫 Friendly fire prevented: ${source.sourceType} from ${source.sourceId}`);
            return {
        ...
        if (targetPartInfo) {
            ...
            console.log(`🎯 Precise damage: componentId=${targetPartInfo.componentId}, partIndex=${targetPartInfo.partIndex}`);
            ...
            console.log(`🎯 Position-based damage at (${collisionInfo.contactPoint.x}, ${collisionInfo.contactPoint.y})`);
