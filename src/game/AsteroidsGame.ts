...
export class AsteroidsGame {
    ...
    private createAsteroid(size: 'large' | 'medium' | 'small', x: number, y: number): void {
        if (!this.engine) return;
        ...
        const asteroidSize = sizeMap[size];        const entity = this.engine.createCircle({
            x,
            y,
        ...
        // Random velocity
        const speed = this.ASTEROID_MIN_SPEED + Math.random() * (this.ASTEROID_MAX_SPEED - this.ASTEROID_MIN_SPEED);
    ...
    private updatePlayer(deltaTime: number): void {
        ...
        if (this.playerThrust) {
            const thrustX = Math.cos(this.playerRotation) * this.PLAYER_THRUST * deltaTime;
            const thrustY = Math.sin(this.playerRotation) * this.PLAYER_THRUST * deltaTime;
        ...
        if (speed > this.PLAYER_MAX_SPEED) {
            this.playerVelocity.x = (this.playerVelocity.x / speed) * this.PLAYER_MAX_SPEED;
            this.playerVelocity.y = (this.playerVelocity.y / speed) * this.PLAYER_MAX_SPEED;
        }
        ...
        if (this.engine) {
            ...
            const playerBody = allBodies.find(body => body.id === this.playerShip!.physicsBodyId);
            if (playerBody) {
    ...
    private fireBullet(): void {
        ...
        if (!this.lastBulletTime || now - this.lastBulletTime > 150) {
            const bulletX = this.playerShip.position.x + Math.cos(this.playerRotation) * 25;
            const bulletY = this.playerShip.position.y + Math.sin(this.playerRotation) * 25;            const entity = this.engine.createCircle({
                x: bulletX,
                y: bulletY,
                radius: 2,
                options: {
    ...
    private updateBullets(deltaTime: number): void {
        ...
        this.bullets.forEach((bullet, index) => {
            ...
            if (this.engine) {
                const physicsSystem = this.engine.getPhysicsSystem();
                const allBodies = physicsSystem.getAllBodies();
                const bulletBody = allBodies.find(body => body.id === bullet.entity.physicsBodyId);
                if (bulletBody) {
    ...
    private updateAsteroids(deltaTime: number): void {
        ...
        this.asteroids.forEach(asteroid => {
            ...
            if (this.engine) {
                const physicsSystem = this.engine.getPhysicsSystem();
                const allBodies = physicsSystem.getAllBodies();
                const asteroidBody = allBodies.find(body => body.id === asteroid.entity.physicsBodyId);
                if (asteroidBody) {
    ...
    private checkCollisions(): void {
        ...
        this.bullets.forEach((bullet, bulletIndex) => {
            this.asteroids.forEach((asteroid, asteroidIndex) => {
                ...
                const asteroidRadius = asteroid.size === 'large' ? 20 : asteroid.size === 'medium' ? 12.5 : 7.5;
        ...
        const uniqueAsteroids = [...new Set(asteroidsToRemove)].sort((a, b) => b - a);
    ...
    private breakAsteroid(asteroid: Asteroid): void {
        if (asteroid.size === 'large') {
            ...
            for (let i = 0; i < 2; i++) {
                this.createAsteroid('medium', asteroid.entity.position.x, asteroid.entity.position.y);
            }
        } else if (asteroid.size === 'medium') {
            // Create 2 small asteroids
            for (let i = 0; i < 2; i++) {
                this.createAsteroid('small', asteroid.entity.position.x, asteroid.entity.position.y);
            }
        }
