...
export class ShipSplitter {
   ...
   */ public static splitShipComposite(
    ...
    console.log(
      `🔧 Starting ship splitting analysis for ${blocks.length} blocks`
    );
    ...
    if (blocks.length === 1) {
      ...
      console.log(
        `🔧 Single block remaining: ${singleBlock.type} (${hasCockpit ? 'ship' : 'debris'})`
      );
    ...
    for (const group of groups) {
      ...
      console.log(
        `🔧 Group of ${group.length} blocks classified as ${hasCockpit ? 'ship' : 'debris'}`
      );
    }

    console.log(
      `🔧 Split result: ${newShips.length} ships, ${debris.length} debris groups`
    );
   ...
   */
  private static createGridLookupMap(
    blocks: ShipBlock[]
  ): Map<string, ShipBlock> {
   ...
   */
  public static groupHasBlockType(
    group: ShipBlock[],
    blockType: string
  ): boolean {
  ...
  public static validateGroupConnectivity(group: ShipBlock[]): boolean {
    ...
    if (!isValid) {
      console.warn(
        `⚠️ Group validation failed: ${visited.size} reachable blocks out of ${group.length} total blocks`
      );
    }
