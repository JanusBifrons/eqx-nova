import type { Vector2D, IPhysicsBody, CompoundBodyPart } from '../../engine/interfaces/IPhysicsSystem';
...
export class ComplexModularShipSplitter {
    ...
    public static splitComplexModularShip(
        ...
        newShips: { body: IPhysicsBody; blocks: BlockConfig[]; renderIds: string[] }[];
        debris: { body: IPhysicsBody; blocks: BlockConfig[]; renderIds: string[] }[];
    } {
        console.log(`🔧 Splitting ComplexModularShip with ${blockConfigs.length} blocks`);
        ...
        const originalBody = physicsSystem.getAllBodies().find(body => body.id === originalBodyId);
        if (originalBody) {
        ...
        const splitResult = ShipSplitter.splitShipComposite(shipBlocks, physicsSystem);
        ...
        console.log(`🔧 Created ${newShips.length} new ships and ${debris.length} debris groups`);
    ...
    private static createNewShipComposites(
        ...
        for (let i = 0; i < shipGroups.length; i++) {
            ...
            const worldCenterX = originalPosition.x + (centerOfMass.x * 16 * cos - centerOfMass.y * 16 * sin);
            const worldCenterY = originalPosition.y + (centerOfMass.x * 16 * sin + centerOfMass.y * 16 * cos);
    ...
    private static createCompoundBodyFromBlocks(
        ...
        return physicsSystem.createCompoundBody(
            position.x,
            position.y,
            parts,
            {
    ...
    private static createRenderObjectsForBlocks(
        ...
        for (let i = 0; i < blockConfigs.length; i++) {
            ...
            const blockWorldX = position.x + (config.offset.x * cos - config.offset.y * sin);
            const blockWorldY = position.y + (config.offset.x * sin + config.offset.y * cos);

            const color = blockTypeColors[config.type] || (isDebris ? 0x333333 : 0x888888);
