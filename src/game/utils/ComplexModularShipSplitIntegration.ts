...
export class ComplexModularShipSplitIntegration {
    ...
    public static checkAndHandleShipSplitting(
        ...
        onNewShipCreated?: (shipData: { body: any; blocks: BlockConfig[]; renderIds: string[] }) => void,
        onDebrisCreated?: (debrisData: { body: any; blocks: BlockConfig[]; renderIds: string[] }) => void
    ): boolean {
        ...
        if (blockConfigs.length === 0) {
            ...
            this.cleanupShipResources(currentPhysicsBodyId, renderObjectIds, physicsSystem, rendererSystem);
            return true;
        }
        ...
        if (blockConfigs.length === 1) {
            ...
            if (lastBlock.type !== 'COCKPIT') {
                console.log('🔧 Last remaining block is not a cockpit - ship destroyed');
                this.cleanupShipResources(currentPhysicsBodyId, renderObjectIds, physicsSystem, rendererSystem);
                return true;
            }
            // Single cockpit remains - ship survives but is severely damaged
            console.log('🔧 Only cockpit remains - ship survives but heavily damaged');
            return false;
        }
    ...
    private static shouldSplitShip(blockConfigs: BlockConfig[]): boolean {
        ...
        if (!isConnected) {
            console.log(`🔧 Connectivity check: ${visited.size}/${gridPositions.size} blocks reachable - splitting needed`);
        }
    ...
    private static cleanupShipResources(
        ...
        // Remove physics body
        const physicsBody = physicsSystem.getAllBodies().find(body => body.id === physicsBodyId);
        if (physicsBody) {
    ...
    private static cleanupRenderObjects(
        ...
        for (const renderObjectId of renderObjectIds) {
            try {
                ...
                console.warn(`⚠️ Failed to remove render object ${renderObjectId}:`, error);
            }
        }
    ...
    public static enhancedDamageBlock(
        ...
        const wasDestroyed = originalDamageBlockMethod.call(shipInstance, blockIndex, damage);

        // If a block was destroyed, check if we need to split the ship
        if (wasDestroyed) {
            console.log(`🔧 Block ${blockIndex} destroyed - checking for ship splitting`);

            // Get the remaining blocks (those not destroyed)
            const remainingBlocks = shipInstance._blockConfigs.filter((block: any) => block.health > 0);
            ...
            const wasSplit = this.checkAndHandleShipSplitting(
                ...
                shipInstance._physicsBody?.id || '', shipInstance._renderObjectIds,
                (_newShipData) => {
