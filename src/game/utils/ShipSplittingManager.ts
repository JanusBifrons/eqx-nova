...
import type { IPhysicsSystem, Vector2D } from '../../engine/interfaces/IPhysicsSystem';
    ...
    public static createShipWithSplitting(
        ...
        const splitCallbacks = {
            onNewShipCreated: (shipData: SplitData) => {
                console.log(`🚀 Creating new ship from split with ${shipData.blocks.length} blocks`);
            ...
            onDebrisCreated: (debrisData: SplitData) => {
                console.log(`🗑️ Creating debris from split with ${debrisData.blocks.length} blocks`);
