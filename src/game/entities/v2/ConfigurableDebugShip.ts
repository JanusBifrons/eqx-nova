...
export class ConfigurableDebugShip implements IModularShip {
  ...
  constructor(
    ...
    console.log(
      `🔧 Block positions: ${blockPositions.map(p => `(${p.x.toFixed(1)}, ${p.y.toFixed(1)})`).join(', ')}`
    );
    ...
    console.log(
      `🔧 Block offsets: ${this._blockOffsets.map(o => `(${o.x.toFixed(1)}, ${o.y.toFixed(1)})`).join(', ')}`
    );
  ...
  private createRenderObjects(position: Vector2D): void {
    console.log(
      `🎨 Creating render objects for ${this._blockOffsets.length} blocks at base position (${position.x}, ${position.y})`
    );
    ...
    for (let i = 0; i < this._blockOffsets.length; i++) {
      ...
      console.log(
        `🎨 Creating render object ${i}: ${renderObjectId} at WORLD (${blockWorldX.toFixed(1)}, ${blockWorldY.toFixed(1)}) size ${this._blockSize}x${this._blockSize}`
      );
    ...
    console.log(
      `🎨 Created ${this._renderObjectIds.length} render objects for configurable ship`
    );
  }
    ...
    if (this._isBrokenApart) {
      ...
      if (this._physicsBody && this._renderObjectIds.length > 0) {
        ...
        if (!this._debugUpdateLogged) {
          console.log(
            `🔄 Updating render objects for ship at (${this._physicsBody.position.x}, ${this._physicsBody.position.y})`
          );
