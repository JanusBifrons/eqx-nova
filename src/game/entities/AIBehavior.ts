...
export class AIBehavior implements IAIBehavior {
  ...
  public setTarget(target: Vector2D | ICompositeShip | null): void {
    ...
    if (target && Math.random() < 0.01) { // 1% chance to log
      if ('centerPosition' in target) {
        console.log(`AI ${this._id}: set ship target at (${target.centerPosition.x.toFixed(1)}, ${target.centerPosition.y.toFixed(1)})`);
      } else {
        console.log(`AI ${this._id}: set position target at (${target.x.toFixed(1)}, ${target.y.toFixed(1)})`);
      }
    }
  }
  ...
  public update(deltaTime: number): void {
    ...
    if (now - this._lastTargetScan > 1000) { // Scan every second
      this.scanForTargets();
  ...
  public shouldFire(): boolean {
    ...
    if (Math.random() < 0.001) { // 0.1% chance to log
      console.log(`AI ${this._id}: target distance=${distance.toFixed(1)}, angleDiff=${(angleDiff * 180 / Math.PI).toFixed(1)}°, shouldFire=${shouldFire}`);
    }
  ...
  private applyMovement(deltaTime: number): void {
    ...
    if (this.shouldRotate()) {
      ...
      const newAngle = currentAngle + Math.sign(angleDiff) * Math.min(Math.abs(angleDiff), rotationStep);
