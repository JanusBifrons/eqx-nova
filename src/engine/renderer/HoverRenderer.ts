...
export class HoverRenderer {
  ...
  public showHoverIndicator(entity: Entity): void {
    if (!this.isInitialized || !this.cameraSystem || !this.gameContainer)
      return;
  ...
  private calculateEntityBoundingSize(entity: Entity): {
    width: number;
    height: number;
  } {
...
return {
  ...
  private calculateDynamicCornerLength(boundingSize: {
    width: number;
    height: number;
  }): number {
    ...
    return Math.max(
      ...
      Math.min(this.MAX_CORNER_LENGTH, dynamicLength)
    );
  }

  private drawCorneredSquare(
    center: Vector2D,
    width: number,
    height: number,
    cornerLength: number
  ...
  private drawCornerLines(
    ...
    this.hoverIndicator
      ...
      .stroke({
        width: this.CORNER_THICKNESS,
        color: this.CORNER_COLOR,
        alpha: this.CORNER_ALPHA,
      });
