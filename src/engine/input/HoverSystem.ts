...
export class HoverSystem {
    ...
    public updateHover(screenMousePosition: Vector2D | null): Entity | null {
        if (!this.isInitialized || !this.physicsSystem || !this.cameraSystem || !screenMousePosition) {
            this.hoveredEntity = null;
            return null;
        }
        ...
        if (Math.random() < 0.01) { // Log occasionally to avoid spam
            console.log('🔍 Hover system: mouse at screen', screenMousePosition, 'world', worldPosition);
        }
    ...
    private findEntityAtPosition(worldPosition: Vector2D): Entity | null {
        ...
        for (const body of allBodies) {
            ...
            if (this.isPointInBodyBounds(worldPosition, body)) {
                ...
                if (entity) {
                    console.log('🎯 Found hoverable entity:', entity.id, 'type:', entity.type);
                    return entity;
                }
            }
        }
