...
export class MouseInteractionSystem {
    ...
    public initialize(
        ...
        if (!element) {
            ...
            console.log('🖱️ MouseInteractionSystem: Using native Matter.js mouse handling with coordinate transformation!');
        }
    ...
    private handleMouseDown(event: MouseInputEvent): void {
        ...
        console.log('🖱️ MOUSE DOWN - Starting drag at screen:', event.position, 'world:', worldPosition);
        this.physicsSystem.updateMouseConstraint(
            this.mouseConstraint,
            worldPosition
        );
        this.isDragging = true;
        console.log('🔗 DRAG STARTED - Mouse constraint activated with MAX STRENGTH!');
    }
    ...
    private handleMouseMove(event: MouseInputEvent): void {
        ...
        if (Math.random() < 0.2) {  // 20% chance to log (even more frequent)
            console.log('🎯 DRAGGING ACTIVE - Moving to world position:', worldPosition, '- CONSTRAINT IS PULLING!');
        }
    }
